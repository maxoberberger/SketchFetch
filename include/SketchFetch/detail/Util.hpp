...
namespace SketchFetch::Util
{
	auto static printResponse(json response) -> void
	{
		for (auto&& i : response["results"])
		{
			fmt::print("\"{}\"{}\n\tViews: {}, Likes: {}\n\tDescription:{:.80}\n", i["name"].get<std::string>(),
				i["uid"].get<std::string>(), i["viewCount"].get<int>(), i["likeCount"].get<int>(),
	...
	public:
		Timer(const std::source_location a_location = std::source_location::current()) : location(a_location)
		{
